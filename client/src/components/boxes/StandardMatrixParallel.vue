<template>
  <div class="card">
    ...
    >
      <template slot="items" slot-scope="props" class="tablemetrix">
        <tr>
          <td>{{ props.item.classifier }}</td>
          <td>{{ `${props.item.metrics.accuracy.toFixed(3)}` }}</td>
          <td>{{ `${props.item.metrics.mcc.average.toFixed(3)}` }}</td>
          <td class="column3">{{ `${props.item.metrics.microf1.average.toFixed(3)}` }}</td>
          <td class="column4">{{ `${props.item.metrics.macrof1.average.toFixed(3)}` }}</td>
          <!--<td class="column5">{{ `${props.item.metrics.f1.average.toFixed(3)}` }}</td>-->
        </tr>
      </template>
    </v-data-table>
  </div>
  ...
  computed: {
    metrics(): MetricsWithClassifier[] {
      ...
      if (this.classes.length <= 2) {
        d3.select(".column3").data(metrics).enter().html(function(d) {return d.metrics["f1"].average.toFixed(3)})
        d3.select(".column4").data(metrics).enter().html(function(d) {return d.metrics["precision"].average.toFixed(3)})
        d3.select(".tablemetrix").append("td").attr("class", "column5").data(metrics).enter().html(function(d) {return d.metrics["recall"].average.toFixed(3)})
      } 
      return metrics;
    },
    classes(): string[] {
